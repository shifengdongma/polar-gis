...
### 数据管理
- **S-57 批量导入**：支持 `.000` 基座 + `.001`/`.002` 顺序更新链，自动识别校验，暂停/恢复/取消控制，最多 5000 文件，ThreadPoolExecutor 并行处理
- **全球海图底图一键导入**：预检 → 分批导入 → 图层组发布 → WMTS 注册 → 缓存预热全自动化
- **数据集版本管理**：父子版本链，支持更新回退与历史追溯
- **多格式支持**：矢量数据 (S-57/Shapefile/GeoJSON) 与栅格数据 (GeoTIFF) 导入
- **数据清理**：软删除 + 级联清理（GeoServer 图层 + PostGIS 表 + 文件资产），二次确认防误删
...
| 系统 | `/health` | 健康检查 + 审计日志 |
| S-57 底图 | `/admin/s57-basemaps` | 全球海图底图预检/导入/状态 |
| GWC 回填 | `/admin/gwc/backfill` | EPSG:3413 缓存批量补齐 |
| 样式刷新 | `/admin/styles/refresh-s57` | SLD 幂等刷新 + 瓦片缓存截断 |
