...
def test_render_sld_with_min_scale_denominator() -> None:
    preset = preset_for_object_class("SOUNDG")
    assert preset is not None
    sld = preset.render_sld(min_scale_denominator=25000.0)
    ...
    assert sld.index("MinScaleDenominator") < sld.index("PointSymbolizer")
